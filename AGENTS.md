...
- Change: Resolved ESLint errors for AI builder, webhook handlers, and preview/build utilities.
...
- Change: Added Prisma client generation on install to fix Vercel build cache issue.
- Files: package.json
- Notes: `postinstall` now runs `prisma generate` before `next build`.
- Next: Trigger a new Vercel build to confirm Prisma error is resolved.

## 2026-01-30 12:10

- Change: Made Supabase optional at build time and added a “coming soon” fallback for /ai when envs are missing.
